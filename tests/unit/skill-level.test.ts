...
describe("skill level helpers", () => {
  it("mapea ranks 1..21 a 7 niveles con distribucion ntile", () => {
  ...
  it("normaliza valores invalidos a un nivel seguro", () => {
    ...
    expect(normalizeSkillLevel(Number.NaN)).toBe(7);
  });

  it("formatea etiquetas humanas para los selectores de nivel", () => {
    expect(formatSkillLevelLabel(1)).toBe("Nivel 1 - Estrella");
    expect(formatSkillLevelLabel(2)).toBe("Nivel 2 - Figura");
    expect(formatSkillLevelLabel(3)).toBe("Nivel 3 - Muy bueno");
    expect(formatSkillLevelLabel(4)).toBe("Nivel 4 - Bueno");
    expect(formatSkillLevelLabel(5)).toBe("Nivel 5 - Intermedio");
    expect(formatSkillLevelLabel(6)).toBe("Nivel 6 - Recreativo");
    expect(formatSkillLevelLabel(7)).toBe("Nivel 7 - Principiante");
  });
  ...
  it("agrega una escala especial para invitados superiores al Nivel 1", () => {
    ...
    expect(parseGuestSkillLevelValue("7")).toBe(7);
    expect(parseGuestSkillLevelValue("8")).toBeNull();
    expect(formatGuestSkillLevelLabel(GUEST_FEATURED_SKILL_LEVEL)).toBe(
      "Invitado superior - mejor que Estrella"
    );
    expect(calculateGuestSkillScore(GUEST_FEATURED_SKILL_LEVEL)).toBe(725);
    expect(calculateGuestSkillScore(1)).toBe(700);
