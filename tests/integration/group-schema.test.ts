...
describe.skipIf(!existsSync(source))("groups database and RLS", () => {
  let db: PGlite;
  beforeAll(async () => {
    db = new PGlite();
    ...
    for (const file of [source, "supabase/generated/policies.app_prod.sql", "supabase/generated/policies.storage.sql"]) {
      ...
      await db.exec(sql);
      await db.exec(sql); // Idempotent deployment is part of the contract.
    }
  }, 30_000);
  ...
  it("allows an invited administrator to create one owned group and blocks a second", async () => {
    ...
    await db.exec(`insert into app_prod.organizations(name,slug,created_by) values ('Own','own','${id(2)}')`);
    await expect(db.exec(`insert into app_prod.organizations(name,slug,created_by) values ('Second','second','${id(2)}')`)).rejects.toThrow(/row-level security/);
  ...
  it("creates matches without recursive policies and rejects cross-group links", async () => {
    ...
    await expect(db.exec(`update app_prod.matches set confirmed_option_id='${id(50)}' where id='${id(41)}'`)).rejects.toThrow(/matches_confirmed_option_ownership_fkey/);
    await db.exec(`update app_prod.matches set confirmed_option_id='${id(51)}' where id='${id(41)}'`);
  });
